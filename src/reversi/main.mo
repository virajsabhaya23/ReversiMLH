...
actor Reversi {
  ...
  func insert_new_player(id: PlayerId, name_: PlayerName) : PlayerState {
    let player = { name = name_; var score = 0; var games_played = 0 };
  ...
  public shared (msg) func start(opponent_name: Text,
  board_dimension: Nat): async Result<GameView, Types.StartError> {
    let player_id = msg.caller;
    switch (lookup_player_by_id(player_id)) {
      ...
      case (?player) {
        if (player.name == opponent_name) {
          ...
          return #ok(proceed(add_game(player_id, player.name, opponent_name, board_dimension)));
        ...
        switch (lookup_game_by_name(player.name), lookup_game_by_name(opponent_name)) {
          ...
          case (?game, null) {
            if (Utils.eq_nocase(game.white.1, player.name)) {
              ...
              game.white := (null, "");
              #ok(proceed(add_game(player_id, player.name, opponent_name, board_dimension)));
            } else {
  ...
  func add_game(black_id: PlayerId, black_name: Text, white_name: Text, board_dimension: Nat) : GameState {
    ...
    let set_games_played = 
      ...
      set_games_played(Option.chain(?black_name, lookup_game_by_name));
      set_games_played(Option.chain(?white_name, lookup_game_by_name));
      ...
      case (?black_player, ?white_player) {
        ...
        let set_score =
          func (player: PlayerState, points: Int, player_level: Nat, opponent_level: Nat) {
            ...
            Utils.update_top_players(top_players, player.name, player.score, player.games_played);
          };
